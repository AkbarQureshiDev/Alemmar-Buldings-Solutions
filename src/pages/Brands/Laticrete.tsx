...
import LaticreteProducts from '../../data/Laticrete';

const Laticrete: React.FC = () => {
  const handleWhatsAppClick = (productName: string) => {
    const message = `I want to buy the ${productName} from Laticrete products`;
      ...
      <div className="text-center mb-16">
        <div className="flex items-center justify-center gap-4 mb-2">
          ...
          <h2 className="text-2xl md:text-3xl font-black text-center tracking-tighter uppercase italic text-black/90">
            Laticrete <span className="text-[#6B5E18] ml-2">Products</span>
          </h2>
          <div className="h-[1px] bg-gray-200 flex-grow max-w-[100px]"></div>
        </div>
        <p className="text-gray-500 text-base max-w-2xl mx-auto leading-relaxed">
          Discover premium LATICRETE® tile adhesives, grouts, sealants, and waterproofing systems engineered for strength. Find professional-grade materials for every surface preparation and installation project.
        </p>
      </div>

      {/* Grid */}
      <div className="max-w-[1400px] mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12">
        {LaticreteProducts.map((product) => (
